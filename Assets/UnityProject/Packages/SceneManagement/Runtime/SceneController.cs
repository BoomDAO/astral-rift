...
namespace Gamekit3D
{
    ...
    public class SceneController : MonoBehaviour
    {
        ...
        {
            UResult<DataTypes.Entity, string> entityResult = null;
            if (SceneManager.GetActiveScene().name == "Level1")
            {
                entityResult = UserUtil.GetElementOfType<DataTypes.Entity>($"{Env.CanisterIds.WORLD}{""}level00_checkpoint");

                if(entityResult.IsOk)
                {
                    var entityAttribute = entityResult.AsOk().attribute;
                    if (Checkpoint.Checkpoints.TryGetValue(entityAttribute, out var checkpoint))
                    {
                        if (checkpoint.TryGetComponent<SceneTransitionDestination>(out var entrance)) return entrance;
                    }
                }
            }
            else if (SceneManager.GetActiveScene().name == "Level2")
            {
                entityResult = UserUtil.GetElementOfType<DataTypes.Entity>($"{Env.CanisterIds.WORLD}{""}level01_checkpoint");
