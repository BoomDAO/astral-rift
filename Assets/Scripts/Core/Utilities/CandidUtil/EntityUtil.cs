...
internal static class EntityUtil
{
    ...
    {
        var wid = actionConfig.Wid.ValueOrDefault;
        if (string.IsNullOrEmpty(wid)) wid = Env.CanisterIds.WORLD;

        return $"{wid}{actionConfig.Gid}{actionConfig.Eid}";
    }
    ...
    // GET QUANTITY
    public static double GetCurrentQuantity(string entityKey)
    {
    ...
    //NEW
    public static bool TryGetConfig(string entityKey, out DataTypes.EntityConfig entityConfig)
    {
        entityConfig = default;
        var config = UserUtil.GetElementOfType<DataTypes.EntityConfig>(entityKey);
    ...
    public static bool TryGetConfig(this Entity entity, out DataTypes.EntityConfig entityConfig)
    {
        return TryGetConfig(entity.GetKey(), out entityConfig);
    }

    //NEW
    public static bool TryGetConfig(this DataTypes.Entity entity, out DataTypes.EntityConfig entityConfig)
    {
        return TryGetConfig(entity.GetKey(), out entityConfig);
    }
    ...
    public static UResult<bool, string> MeetEntityRequirements(params ActionConstraint.EntityConstraintItemItem[] constrains)
    {
        ...
        foreach (var constrain in constrains)
        {
            ...
            {
                if (ownEntity.GetKey() == constrain.GetKey())//FIX 
                {
                    int a = 0, b = 0;
    ...
    //NEW
    public static double SetCurrentAttribute(params DataTypes.Entity[] valuesToIncrementBy)
    {
        ...
        {
            var newValue = valuesToIncrementBy[i];
            var currentValue = UserUtil.GetElementOfType<DataTypes.Entity>(newValue.GetKey());

            if (currentValue.IsOk)
            {
                var asOk = currentValue.AsOk();
                asOk.attribute = newValue.attribute;
                newValues.Add(asOk);
            }
        }
    ...
    public static double SetCurrentQuantity(params DataTypes.Entity[] valuesToIncrementBy)
    {
        List<DataTypes.Entity> newValues = new();
        double currentQuantity = 0;
    ...
    public static double IncrementCurrentQuantity(params DataTypes.Entity[] valuesToIncrementBy)
    {
        ...
        for (int i = 0; i < valuesToIncrementBy.Length; i++)
        {
            var element = valuesToIncrementBy[i];
            currentQuantity = UserUtil.GetPropertyFromType<DataTypes.Entity, double>(element.GetKey(), e => e.quantity.GetValueOrDefault(), 0);
            Debug.Log($"> > > Current quantity of {element.GetKey()}: {currentQuantity}");
            if(element.quantity != null)
            {
                element.quantity += currentQuantity;

                Debug.Log($"> > > New quantity of {element.GetKey()}: {element.quantity}");
