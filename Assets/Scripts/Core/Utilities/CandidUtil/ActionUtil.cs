...
public static class ActionUtil
{
    ...
    public static bool TryGetTriesLeft(string actionId, out ulong triesLeft)
    {
        ...
        var asOk = actionConfigResult.AsOk();
        ...
        if (asOk.timeConstraint.ActionsPerInterval.TryToUInt64(out var actionsPerIntervalConfig) == false) {
            Debug.Log("Converting \"timeConstrain.ActionsPerInterval\" to UInt64 failed");
            return false;
        }

        if (actionsPerIntervalConfig == 0)
        {
            return false;
        }

        if (asOk.timeConstraint.IntervalDuration.TryToUInt64(out var intervalDurationConfig) == false) {
        ...
        {
            triesLeft = actionsPerIntervalConfig;
            return true;
        }
        else if (actionCount < actionsPerIntervalConfig)
        {
            triesLeft = actionsPerIntervalConfig - actionCount;
            return true;
        }
        else
        {
    ...
    public static UResult<Null, ActionErrType.Base> ValidateActionConfig(string actionId)
    {
        return ValidateActionConfig(actionId, out var outVal);
    }
